...
export class MonsterPageComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    console.log({'id pokemon': this.idMonster, 'generation' : this.idPokeGen, 'id Endoint pokedex' : this.idDex});
    console.log(this.pokedexService.getCurrentPokedexData(parseInt(this.idDex)));
  }

  ngOnInit(): void {
    this.monsterDetails$ = this.pokeApi.getPokemonDetails(this.idMonster).pipe(
      tap((data) => console.log(data)),
      tap((pokemonFullData => this.setPokemonSprite$(pokemonFullData.sprites, this.idPokeGen)))
    );
  }

  setPokemonSprite$(spriteObject: PokemonSprites, generation: string): void {
    // console.log("Into setPokemonSprite$ ", spriteObject, generation);
    this.pokemonPageService.getPokemonArtworkByIdGeneration(spriteObject, generation);
    // this.pokemonSelectedSprite = this.pokemonPageService.getPokemonArtworkByIdGeneration(spriteObject, generation);
  }
