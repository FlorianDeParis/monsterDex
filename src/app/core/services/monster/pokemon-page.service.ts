...
export class PokemonPageService {
  ...
  getPokemonArtworkByIdGeneration(spriteObject: PokemonSprites, IdGen: string): string{
    ...
    if(typeof spriteObject.versions !== "undefined"){
      ...
      if(spriteWithGen){
        const indexesWithoutIcons = Object.keys(spriteWithGen).filter((index) => index != "icons"); // Ignore icon object when fetching sprites
        if(indexesWithoutIcons.length > 0){
          spriteFront = this.getMyObjectValueCastedKey(spriteWithGen[indexesWithoutIcons[0]], 'front_default');
        }
      }
      if(spriteWithGen && spriteFront){
        returnedSprite = spriteFront;
      } else {
        this.toaster.error('Sprite non trouvé, chargement du sprite par défaut');
      }
    }
